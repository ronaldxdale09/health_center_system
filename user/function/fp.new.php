...
if (isset($_POST['new'])) {
    ...
    $query = "INSERT INTO family_planning_rec (dateRecording) 
                            VALUES (NOW())";
    ...
    if ($results) {
        ...
        header("Location: ../family_planning.php?id=$last_id"); // Change this to your desired location
        exit();
    } else {
...
if (isset($_POST['delete'])) {
    ...
    $query = "DELETE FROM family_planning_rec WHERE fp_id  = ?";
    if ($stmt = $con->prepare($query)) {
        $stmt->bind_param("i", $record_id); 

        if ($stmt->execute()) {
            header("Location: ../fp_record.php"); 
