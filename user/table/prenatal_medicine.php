...
$query = "SELECT * FROM prenatal_medicine WHERE prenatal_id = '$prenatal_id'";
...
<table class="table table-hover table-bordered table-striped "  id="prenatal_medicine_list" >
...
// Output the table rows from the prenatal_medicine query
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $medDropdown .= '</select>';
     ...
     <td hidden><input type="text" class="form-control payment_id" name="med_id[]" value="' . htmlspecialchars($row['idNo']) . '"></td>
         ...
         <td>
             <div class="input-group">
                 <button type="button" class="btn btn-danger btn-sm remove-item-line removeList">Remove</button>
             </div>
         </td>
     </tr>';
...
<script>
    $(document).ready(function () {
        ...
        `;
            ...
            $("#prenatal_medicine_list tbody").append(newRow);
        });

        // Event delegation for dynamically created select dropdowns
        $("#prenatal_medicine_list tbody").on('change', '.med-dropdown', function () {
